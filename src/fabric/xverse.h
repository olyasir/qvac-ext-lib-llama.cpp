...
struct XverseDecoderLayer {
    XverseAttention self_attn;
    XverseMLP mlp;
    RMSNorm attn_norm, ffn_norm;

    XverseDecoderLayer() = default;
    XverseDecoderLayer(const llama_layer & l, int64_t hd, int64_t nh, int64_t nhkv)
...
struct XverseModel {
    ...
    std::vector<XverseDecoderLayer> layers;
    RMSNorm norm;

    XverseModel() = default;
    XverseModel(const llama_model & m, int64_t hd, int64_t nh, int64_t nhkv) {
...
struct XverseForCausalLM {
    XverseModel model;
    ggml_tensor * lm_head = nullptr;

    XverseForCausalLM() = default;
    XverseForCausalLM(const llama_model & m, int64_t hd, int64_t nh, int64_t nhkv)
