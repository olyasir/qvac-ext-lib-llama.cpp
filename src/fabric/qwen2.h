...
struct Qwen2MLP {
    ...
    Qwen2MLP() = default;
    Qwen2MLP(const llama_layer & l) : gate(l.ffn_gate), up(l.ffn_up), down(l.ffn_down) {}
...
struct Qwen2Attention {
    ...
    Qwen2Attention(const llama_layer & l, int64_t hd, int64_t nh, int64_t nhkv)
...
struct Qwen2DecoderLayer {
    ...
    Qwen2DecoderLayer() = default;
    Qwen2DecoderLayer(const llama_layer & l, int64_t hd, int64_t nh, int64_t nhkv)
...
struct Qwen2Model {
    ...
    Qwen2Model() = default;
    Qwen2Model(const llama_model & m, int64_t hd, int64_t nh, int64_t nhkv) {
...
struct Qwen2ForCausalLM {
    ...
    Qwen2ForCausalLM() = default;
    Qwen2ForCausalLM(const llama_model & m, int64_t hd, int64_t nh, int64_t nhkv)
        : model(m, hd, nh, nhkv), lm_head(m.output), output_b(m.output_b) {}
    ...
    void forward(GraphContext & ctx) const {
        ...
        if (output_b) {
            logits = logits + Tensor(output_b, &ctx);
        }
