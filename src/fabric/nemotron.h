...
// ============================================================
// Nemotron — LayerNorm, optional biases, ReLU² sequential FFN
// ============================================================
...
struct NemotronMLP {
    ...
    NemotronMLP() = default;
    NemotronMLP(const llama_layer & l)
...
struct NemotronAttention {
    ...
    NemotronAttention() = default;
    NemotronAttention(const llama_layer & l, int64_t hd, int64_t nh, int64_t nhkv)
...
struct NemotronDecoderLayer {
    NemotronAttention self_attn;
    NemotronMLP mlp;
    LayerNorm attn_norm, ffn_norm;

    NemotronDecoderLayer() = default;
    NemotronDecoderLayer(const llama_layer & l, int64_t hd, int64_t nh, int64_t nhkv)
    ...
    Tensor forward(GraphContext & ctx, Tensor x, Tensor pos,
        ...
        x = mlp.forward(ctx, x, il);
        x = x + residual; ctx.name(x, "ffn_out", il);
...
struct NemotronModel {
    ...
    std::vector<NemotronDecoderLayer> layers;
    LayerNorm norm;

    NemotronModel() = default;
    NemotronModel(const llama_model & m, int64_t hd, int64_t nh, int64_t nhkv) {
...
struct NemotronForCausalLM {
    NemotronModel model;
    ggml_tensor * lm_head = nullptr;

    NemotronForCausalLM() = default;
    NemotronForCausalLM(const llama_model & m, int64_t hd, int64_t nh, int64_t nhkv)
