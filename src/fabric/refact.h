...
struct RefactDecoderLayer {
    RefactAttention self_attn;
    RefactMLP mlp;
    RMSNorm attn_norm, ffn_norm;

    RefactDecoderLayer() = default;
    RefactDecoderLayer(const llama_layer & l, int64_t hd, int64_t nh, int64_t nhkv)
    ...
    Tensor forward(GraphContext & ctx, Tensor x,
        ...
        x = self_attn.forward(ctx, x, kv, il);
...
struct RefactModel {
    ...
    std::vector<RefactDecoderLayer> layers;
    RMSNorm norm;

    RefactModel() = default;
    RefactModel(const llama_model & m, int64_t hd, int64_t nh, int64_t nhkv) {
    ...
    Tensor forward(GraphContext & ctx) const {
        ...
        for (int il = 0; il < (int)layers.size(); ++il)
            x = layers[il].forward(ctx, x, kv, out_ids, il, il == (int)layers.size()-1);
        x = norm.forward(ctx, x, -1); ctx.name(x, "result_norm", -1);
        return x;
    }
};

struct RefactForCausalLM {
    RefactModel model;
    ggml_tensor * lm_head = nullptr;

    RefactForCausalLM() = default;
    RefactForCausalLM(const llama_model & m, int64_t hd, int64_t nh, int64_t nhkv)
