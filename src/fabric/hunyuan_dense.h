...
// ============================================================
// HunyuanDense — Like Exaone but QK-norm applied AFTER RoPE
// Uses learned RMSNorm weights (attn_q_norm, attn_k_norm)
// ============================================================

struct HunyuanDenseMLP {
    ggml_tensor * gate = nullptr, * up = nullptr, * down = nullptr;
    HunyuanDenseMLP() = default;
    HunyuanDenseMLP(const llama_layer & l) : gate(l.ffn_gate), up(l.ffn_up), down(l.ffn_down) {}
    Tensor forward(GraphContext & ctx, Tensor x, int il) const {
        return ctx.ffn(x, up, gate, down, LLM_FFN_SILU, LLM_FFN_PAR, il);
    }
};

struct HunyuanDenseAttention {
    ...
    HunyuanDenseAttention() = default;
    HunyuanDenseAttention(const llama_layer & l, int64_t hd, int64_t nh, int64_t nhkv)
        : q_proj(l.wq, l.bq), k_proj(l.wk, l.bk), v_proj(l.wv, l.bv)
        , q_norm(l.attn_q_norm), k_norm(l.attn_k_norm)
    ...
    Tensor forward(GraphContext & ctx, Tensor x, Tensor pos,
        ...
        // RoPE first, then Q/K normalization
        Q = ctx.rope(Q, pos, rope_factors);
        ctx.name(Q, "Qcur", il); ctx.name(K, "Kcur", il); ctx.name(V, "Vcur", il);
        K = ctx.rope(K, pos, rope_factors);
        K = k_norm.forward(ctx, K, il); ctx.name(K, "Kcur_norm", il);
        Q = q_norm.forward(ctx, Q, il); ctx.name(Q, "Qcur_norm", il);
        return ctx.attn(kv, o_proj, o_proj_b, Q, K, V, 1.0f/sqrtf(float(head_dim)), il);
    }
};

struct HunyuanDenseDecoderLayer {
    HunyuanDenseAttention self_attn;
    HunyuanDenseMLP mlp;
    RMSNorm attn_norm, ffn_norm;

    HunyuanDenseDecoderLayer() = default;
    HunyuanDenseDecoderLayer(const llama_layer & l, int64_t hd, int64_t nh, int64_t nhkv)
...
struct HunyuanDenseModel {
    const llama_model * mdl = nullptr;
    ggml_tensor * embed_tokens = nullptr;
    std::vector<HunyuanDenseDecoderLayer> layers;
    RMSNorm norm;

    HunyuanDenseModel() = default;
    HunyuanDenseModel(const llama_model & m, int64_t hd, int64_t nh, int64_t nhkv) : mdl(&m) {
...
struct HunyuanDenseForCausalLM {
    HunyuanDenseModel model;
    ggml_tensor * lm_head = nullptr;

    HunyuanDenseForCausalLM() = default;
    HunyuanDenseForCausalLM(const llama_model & m, int64_t hd, int64_t nh, int64_t nhkv)
