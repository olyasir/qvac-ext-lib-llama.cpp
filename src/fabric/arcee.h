...
// ============================================================
// Arcee — Like LLaMA but with ReLU² sequential FFN (no gate)
// ============================================================

struct ArceeMLP {
    ggml_tensor * up = nullptr, * down = nullptr;
    ArceeMLP() = default;
    ArceeMLP(const llama_layer & l) : up(l.ffn_up), down(l.ffn_down) {}
    Tensor forward(GraphContext & ctx, Tensor x, int il) const {
        return ctx.ffn(x, up, nullptr, nullptr, down, nullptr, nullptr,
                       LLM_FFN_RELU_SQR, LLM_FFN_SEQ, il);
    }
};

struct ArceeAttention {
    ...
    ArceeAttention() = default;
    ArceeAttention(const llama_layer & l, int64_t hd, int64_t nh, int64_t nhkv)
    ...
    Tensor forward(GraphContext & ctx, Tensor x, Tensor pos,
                   ...
                   ggml_tensor * rope_factors, float kq_scale, int il) const {
...
struct ArceeDecoderLayer {
    ArceeAttention self_attn;
    ArceeMLP mlp;
    RMSNorm attn_norm, ffn_norm;

    ArceeDecoderLayer() = default;
    ArceeDecoderLayer(const llama_layer & l, int64_t hd, int64_t nh, int64_t nhkv)
        : self_attn(l, hd, nh, nhkv), mlp(l)
        , attn_norm(l.attn_norm), ffn_norm(l.ffn_norm) {}

    Tensor forward(GraphContext & ctx, Tensor x, Tensor pos,
                   llm_graph_input_attn_kv * kv, Tensor out_ids,
                   ggml_tensor * rope_factors, float kq_scale,
                   int il, bool is_last) const {
        Tensor residual = x;
        x = attn_norm.forward(ctx, x, il); ctx.name(x, "attn_norm", il);
        x = self_attn.forward(ctx, x, pos, kv, rope_factors, kq_scale, il);
...
struct ArceeModel {
    ...
    std::vector<ArceeDecoderLayer> layers;
    RMSNorm norm;
    float kq_scale = 0.0f;

    ArceeModel() = default;
    ArceeModel(const llama_model & m, int64_t hd, int64_t nh, int64_t nhkv) : mdl(&m) {
    ...
    Tensor forward(GraphContext & ctx) const {
        ...
        for (int il = 0; il < (int)layers.size(); ++il) {
            ...
            x = layers[il].forward(ctx, x, pos, kv, out_ids, rope_factors,
                                   kq_scale, il, il == (int)layers.size()-1);
        }
...
struct ArceeForCausalLM {
    ArceeModel model;
    ggml_tensor * lm_head = nullptr;

    ArceeForCausalLM() = default;
    ArceeForCausalLM(const llama_model & m, int64_t hd, int64_t nh, int64_t nhkv)
